...
if __name__ == "__main__":
    ...
    elif len(config.TAXONOMIES) == 3:
        if all(x in ['cm', 'pcs', 'drg', 'hcpcs'] for x in config.TAXONOMIES):
            print('3 Taxonomies')
            print(f'weights: {config.WEIGHTS[0]}:{config.WEIGHTS[1]}:{config.WEIGHTS[2]}')
            Processer.process_3_tax()
    
    elif len(config.TAXONOMIES) == 4:
        if all(x in ['cm', 'pcs', 'drg', 'hcpcs'] for x in config.TAXONOMIES):
            print('4 Taxonomies')
            print("TAX_1 = cm, TAX_2 = pcs, TAX_3 = drg, TAX_4 = hcpcs")
            print(f'weights: {config.WEIGHTS[0]}:{config.WEIGHTS[1]}:{config.WEIGHTS[2]}:{config.WEIGHTS[3]}')
            Processer.process_4_tax()

    else:
        raise ValueError("Wrong number of taxonomies. Please insert 1, 2, 3 or 4 taxonomy names in the config file for TAXONOMIES.")
